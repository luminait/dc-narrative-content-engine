...
import { Plus, Loader2, X, Maximize2, AlertCircle } from 'lucide-react';
...
import type { Campaign, Character } from '@/src/lib/types/ui';
import {
    isMediaAssetType,
    isAssetReferenceId,
                                ...
                                <div className="bg-blue-50 p-4 rounded-lg border-l-4 border-blue-400">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {campaignPersonas.map( ( persona, personaIndex ) => (
                                            <Badge
                                                key={persona.key || persona.persona_key || personaIndex}
                                                variant="secondary"
                                                className="bg-blue-100 text-blue-800"
                                            >
                                                {persona.label || persona.persona_label} ({persona.key || persona.persona_key})
                                            </Badge>
                                        ) )}
                                    </div>
                            ...
                            {campaignMergeFields.map( ( field, fieldIndex ) => (
                                <div key={fieldIndex}
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center gap-3 mb-2">
                                                ...
                                                <Badge variant="outline" className="text-xs">
                                                    {field.media_value_type}
                                                </Badge>
                                                {field.merge_field_type && (
                                                ...
                                                <div className="bg-gray-50 p-3 rounded-lg mb-4">
                                                    ...
                                                    {field.start_time && field.end_time && (
                                                        <p className="text-xs text-gray-500 mt-1">
                                                            Timing: {field.start_time}s - {field.end_time}s
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex flex-wrap gap-2">
                                            {( assetPools[field.merge_field] || [] ).map( ( asset, assetIndex ) => {
                                                const {
                                                    displayValue,
                                                    clickableUrl,
                                                    isUrl
                              ...
                              {displayValue}
                                                        ...
                                                        <button
                                                            onClick={( e ) => {
                                                                e.stopPropagation();
                                                                onRemoveAssetFromPool( field.merge_field, assetIndex );
                                                            }}
                                        ...
                                        <div className="flex items-center gap-2">
                                            {!activeInputs[field.merge_field] ? (
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                                    onClick={() => onSetActiveInputs( prev => ( {
                                                        ...prev,
                                                        [field.merge_field]: true
                                                    } ) )}
                                                    className="text-xs"
                                                >
                                                    + Add Manual Entry
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <Input
                                                        ...
                                                        value={inputValues[field.merge_field] || ''}
                                                        onChange={( e ) => onSetInputValues( prev => ( {
                                                            ...prev,
                                                            [field.merge_field]: e.target.value
                                                        } ) )}
                                                        onKeyDown={( e ) => {
                                                            if ( e.key === 'Enter' ) {
                                                                addInputAsset( field.merge_field );
                                                            }
                                                            if ( e.key === 'Escape' ) {
